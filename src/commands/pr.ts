...
export async function prCommand(options: PROptions): Promise<void> {
  ...
  if (options.dryRun) {
    ...
    for (const gap of gaps) {
      ...
      // Show insertion (in green) — use the closing-line indent for exact mutation insertions
      const indentRef = exact ? Math.max(0, targetLine - 2) : targetLine - 1;
 ...
 */
function getEffectiveTargetLine(content: string, gap: TrackingGap): { line: number; exact: boolean } {
...
function insertTracking(content: string, targetLine: number, trackingCode: string, exact = false): string {
  const lines = content.split('\n');
