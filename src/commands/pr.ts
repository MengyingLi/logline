...
import { generateTrackingCode, detectUseMutation } from '../lib/utils/code-generator';
...
export async function prCommand(options: PROptions): Promise<void> {
  ...
  if (options.dryRun) {
    ...
    for (const gap of gaps) {
      ...
      // Show insertion — for mutations inserting before `});`, use the previous line's indent.
  ...
  for (const [relativePath, gaps] of fileToGaps) {
    ...
    for (const gap of sorted) {
      ...
      content = insertTracking(content, targetLine, trackingCode, exact);
...
/**
 * Determine where to insert tracking code and whether to bypass insertion heuristics.
 * For useMutation hooks:
 *   - onSuccess exists → insert inside it (exact, no heuristics)
 *   - onSuccess missing → insert before the mutation's closing `});` (exact, no heuristics)
 * For everything else → existing onClick/handler-definition heuristics.
 */
export function getEffectiveTargetLine(content: string, gap: TrackingGap): { line: number; exact: boolean } {
  const targetLine = gap.location?.line ?? 0;
  if (targetLine <= 0) return { line: targetLine, exact: false };

  // Check for useMutation first — takes precedence over generic heuristics.
  const mutCtx = detectUseMutation(content, targetLine);
  if (mutCtx.found) {
    if (mutCtx.hasOnSuccess && mutCtx.onSuccessBodyLine) {
      return { line: mutCtx.onSuccessBodyLine, exact: true };
    }
    if (mutCtx.mutationClosingLine) {
      return { line: mutCtx.mutationClosingLine, exact: true };
    }
  }
  ...
  if (handlerName) {
    ...
    if (handlerLine) {
      return { line: handlerLine, exact: false };
    }
  }

  return { line: targetLine, exact: false };
}
...
export function insertTracking(content: string, targetLine: number, trackingCode: string, exact = false): string {
  const lines = content.split('\n');
  ...
  // When inserting before a closing `});` (useMutation end), the closing line
  // has less indent than the object keys. Use the previous line's indent instead
  // so the inserted code is at the same level as `mutationFn:`, `onSuccess:` etc.
  const atClosingBrace = exact && /^\s*\}\)/.test(lines[insertLine] ?? '');
  const indentSourceLine = atClosingBrace ? Math.max(0, insertLine - 1) : insertLine;
  const indent = lines[indentSourceLine]?.match(/^(\s*)/)?.[1] ?? '  ';
