...
function updateEvents(plan: TrackingPlan, eventName: string | undefined, all: boolean): { updated: number } {
  let updated = 0;
  const targets = all
    ? new Set(plan.events.filter((e) => e.status === 'suggested' || e.status === 'approved').map((e) => e.id))
    : new Set(plan.events.filter((e) => e.name === eventName && (e.status === 'suggested' || e.status === 'approved')).map((e) => e.id));

  plan.events = plan.events.map((e) => {
    if (!targets.has(e.id)) return e;
    updated += 1;
    return { ...e, status: 'deprecated' as const };
  });
...
export async function rejectCommand(options: {
  cwd?: string;
  eventName?: string;
  all: boolean;
}): Promise<void> {
  ...
  if (!options.all && !options.eventName) {
    console.log(chalk.dim('Usage: logline reject <eventName> OR logline reject --all'));
    return;
  }

  const { updated } = updateEvents(plan, options.eventName, options.all);
  if (updated === 0) {
    console.log(chalk.dim('No matching events to reject. (Hint: only suggested/approved can be rejected.)'));
    return;
  }

  recalcCoverage(plan);
  writeTrackingPlan(cwd, plan);
  console.log(chalk.bold(`✓ Deprecated ${updated} event${updated === 1 ? '' : 's'}`));
}
